...
public class StaticToadlet extends WebPage {
	...
	void handle(String path, String filename, String source, String mime) {
		if(!path.startsWith(WebInterface.PATH + "/static/")) {
			throw new IllegalArgumentException("Path must start with /static/");
		}
		if(!path.endsWith("/")) {
			throw new IllegalArgumentException("Path must end with /");
		}

		if(!source.startsWith("/org/freenetproject/freemail/")) {
			throw new IllegalArgumentException("Source must start with /org/freenetproject/freemail/");
		}
		if(!source.endsWith("/")) {
